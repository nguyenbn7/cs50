...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(-1, 2):
            for j in range(-1, 2):
                if i == j:
                    continue
                curr_cell = (abs(i - cell[0]), abs(j - cell[1]))
                if curr_cell not in self.safes and 0 <= curr_cell[0] < self.height and 0 <= curr_cell[1] < self.width:
                    cells.add(curr_cell)
    ...
    def make_safe_move(self):
        ...
        """
        if len(self.safes) != 0:
            # print(f"minesweeper_line 241: {self.safes}")
    ...
    def make_random_move(self):
        ...
        if self.make_safe_move() == None:
            # print(f"minersweeper_lines 254:({self.width}, {self.height})")
            while len(self.moves_made) <= (self.height * self.width):
                cell = (random.randrange(0, self.height - 1),
